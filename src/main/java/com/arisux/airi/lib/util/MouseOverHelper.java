	...
	public static MovingObjectPosition getMouseOver(float maxDistance, Block[] ignoredBlocks)
	{
		...
		if (mc.renderViewEntity != null)
		{
			if (mc.theWorld != null)
			{
				...
				Vec3 vec33 = null;
				float range = 1.0F;
				List<Entity> list = mc.theWorld.getEntitiesWithinAABBExcludingEntity(mc.renderViewEntity, mc.renderViewEntity.boundingBox.addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand((double) range, (double) range, (double) range));
				double d2 = d1;
