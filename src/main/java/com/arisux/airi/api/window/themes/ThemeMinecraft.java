...
public class ThemeMinecraft extends Theme implements ITheme
{
	...
	{
		RenderLib.drawBlockSide(Blocks.stone_slab, 2, window.getX(), window.getY() - 16, window.getWidth(), 16, window.getWidth() / 50f, 0.5f);
	}

	@Override
	public void drawTitleBar(Window window, int mouseX, int mouseY)
	{
		RenderLib.drawBlockSide(Blocks.bedrock, 0, window.getX(), window.getY(), window.getWidth(), window.getHeight(), window.getWidth() / 50f, window.getHeight() / 50f);
	...
	{
		RenderLib.drawBlockSide(Blocks.tnt, 2, window.getX() + window.getWidth() - 14, window.getY() - 14, 12, 12);
	}
}
