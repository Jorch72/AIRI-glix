...
public class ThemeDefault extends Theme implements ITheme
{
	...
	{
		RenderEngine.drawGradientRect(window.getX(), window.getY() - 2, window.getWidth(), window.getHeight(), 0xFF000000, 0xAA001122);
	}

	@Override
	public void drawTitleBar(Window window, int mouseX, int mouseY)
	{
		RenderEngine.drawRect(window.getX(), window.getY() - 16, window.getWidth() - 15, 14, 0xBB000000);
		RenderEngine.drawString(window.getTitle(), window.getX()  + 5, window.getY() - 12, 0xFF0088FF, false);
	}

	@Override
	public void drawCloseButton(Window window, int mouseX, int mouseY)
	{
		RenderEngine.drawRect(window.getX() + window.getWidth() - 15, window.getY() - 16, 15, 14, 0x22CCCCCC);
		RenderEngine.drawString("x", window.getX() + window.getWidth() - 10, window.getY() - 14, 0x330088FF, false);
	}
}
